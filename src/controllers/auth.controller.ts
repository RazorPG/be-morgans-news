...
export const activation = async (req: Request, res: Response) => {
  try {
    ...
    const activation = await activationAccount(req.body)
    console.log("Activation Result:", activation)
    if (activation === null) {
      return response.error(res, "Invalid activation code", 400)
    }
...
export const verifyOtp = async (req: Request, res: Response) => {
  try {
    ...
    const { code } = req.body
    if (!user?.OTP?.code || !user?.OTP?.expire) {
      return response.error(res, "OTP is not available!", 400)
    }
    ...
    const updatedUser = await findUserByUsernameOrEmail(user.email)
    const token = jwtAssign(updatedUser)
    return response.success(res, "success verify otp", 201, token)
  } catch (error: any) {
    if (error.errors) {
      return response.requestError(res, error.errors[0])
    }
